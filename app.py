...
def add_student():
    if request.method == 'POST':
        ...
        if not name or not roll:
            flash('Name and Roll required','danger')
...
def add_grade():
    ...
    if request.method == 'POST':
        ...
        if not student_id or not subject or score == '':
        ...
        if not student:
            ...
            return redirect(url_for('add_grade'))
        g = Grade(subject=subject, score=s, student=student)
        db.session.add(g)
        db.session.commit()
...
def calculate_averages():
    ...
    results = [{'student': s, 'average': s.average()} for s in students]
    ...
    if request.method == 'POST':
        ...
        if subject:
            ...
            if top:
                ...
                flash('No grades for subject','warning')
...
def backup():
    if request.method == 'POST':
        ...
        if action == 'export':
            students = Student.query.all()
            lines = [
                f"{s.roll_number},{s.name},{';'.join(f'{g.subject}:{g.score}' for g in s.grades)}"
                for s in students
            ]
                ...
                if len(parts) > 2 and parts[2]:
                    for g in parts[2].split(';'):
                        if ':' in g:
                            ...
                            try:
                                scf = float(sc)
                                db.session.add(Grade(subject=sub.strip(), score=scf, student_id=s.id))
                            except:
                                pass
            db.session.commit()
